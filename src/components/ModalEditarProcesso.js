...
class ModalEditarProcesso extends Component {
  constructor(props) {
    super(props)
    this.state = {
  ...
  getCpfdAdvogados = () => {
    fetch(enderecoApi + "funcionario/", {
      ...
      .then((response) => {
        if (response.status !== 200) {
          console.log("GET de Advogados falhou. 00")
        } else {
  ...
  editProcess = () => {
    let usuario = this.state.usuario;
    fetch(enderecoApi + "processo/" + usuario.numeroProcesso, {
      method: "PUT",
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
  ...
  render() {
    return (
      <div>
        ...
        >
          <DialogTitle id="scroll-dialog-title">Editar Processo</DialogTitle>
          <DialogContent>
